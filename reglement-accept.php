...
// If user is not logged in, send to connexion
if (empty($_SESSION['user_id'])) {
    app_redirect('connexion.php');
}

// If already accepted, redirect to homepage
if (!empty($_SESSION['reglement_accepte']) && (int) $_SESSION['reglement_accepte'] === 1) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare('UPDATE account_wtc SET reglement_accepte = 1 WHERE id = ?');
    $stmt->execute([$userId]);
...
<body>
    ...
    <section class="section" id="reglement-app">
        <div class="container">
            ...
            <form method="post" class="mt-4 reglement-accept__form">
                ...
                <button type="submit" id="acceptBtn" class="btn-wtc-gold mt-3" disabled>Valider</button>
            </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            var check = document.getElementById('acceptCheck');
